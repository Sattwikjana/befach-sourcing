...
function isInsideExpoApp() {
  return !!(typeof window !== 'undefined' && window.ReactNativeWebView);
}
function googleSignInAvailable() {
  if (!googleClientId()) return false;
  // Inside the Expo app we use the native bridge to expo-auth-session
  // (see mobile/App.tsx). On the regular web, we use the official GSI
  // redirect flow. Both available — only no-Client-ID hides the button.
  return true;
}
function renderGoogleSignInBlock() {
  if (!googleSignInAvailable()) return '';
  // App-mode: render a plain pill button that posts a message to
  // native, which opens the OS browser, signs the user in, and
  // injects the id_token back via window.__handleGoogleSignInToken().
  // Browser-mode: render the official GSI button div which gets
  // populated by mountGoogleSignInButton().
  if (isInsideExpoApp()) {
    return `
      <div class="auth-google-block">
        <button type="button" id="googleSignInAppBtn" class="auth-google-app-btn">
          <span class="auth-google-app-btn-icon" aria-hidden="true">
          ...
          <span class="auth-google-app-btn-label">Continue with Google</span>
        </button>
...
// Called by the native bridge in mobile/App.tsx after a successful
// Google sign-in. Receives the id_token, posts it to our standard
// /api/auth/google endpoint, refreshes state, navigates to /account.
window.__handleGoogleSignInToken = async function (idToken) {
  if (!idToken) return;
  try {
...
function mountGoogleSignInButton() {
  ...
  // In-app mode — wire our own button to post the request to native.
  if (isInsideExpoApp()) {
    const appBtn = document.getElementById('googleSignInAppBtn');
    if (appBtn) {
      appBtn.onclick = () => {
        try {
          window.ReactNativeWebView.postMessage('GS_GOOGLE_SIGNIN_REQUEST');
        } catch {}
      };
    }
    return;
  ...
  // Browser mode — render the official GSI button.
